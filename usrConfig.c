...
IMPORT STATUS sysSetTlb (ULONG virtAddr,ULONG physAddr,ULONG size,	UINT mode);
...
void usrInit
    ...
    {
     sysSetTlb(0x3ff00000, 0x3ff00000, 0x2000, IO_SPACE_MODE);
     *(unsigned volatile long long *)0x3ff00010=0x40000000;
	 *(unsigned volatile long long *)0x3ff00030=0xffffffffe0000000;
	 *(unsigned volatile long long *)0x3ff00050=0x0;
	 sysSetTlb(0xd0000000, 0x50000000, 0x10000000, IO_SPACE_MODE);
    /*
...
#endif  /* INCLUDE_IDE */
#ifdef  INCLUDE_GPIO/* add by sunchao */

	if(gpioDrv() == ERROR){
		printf("gpioDrv install error!\n");
	}
	gpioDevCreate("/beep/0", Sii0680_baseaddr+0xa4, 21);
	gpioDevCreate("/led/0", Sii0680_baseaddr+0xa4, 23);
	gpioDevCreate("/led/1", Sii0680_baseaddr+0xa4, 22);
	
	tmp = *(volatile unsigned int *)(Sii0680_baseaddr+0xa4);
	tmp |= (1<<21)|(1<<22)|(1<<23)|(1<<30)|(1<<28)|(1<<26);
	tmp &= ~((1<<27)|(1<<29)|(1<<31));
	*(volatile unsigned int *)(Sii0680_baseaddr+0xa4) = tmp;
	
#endif
#ifdef  INCLUDE_ATA
    {                                   /* initialize hard disk driver */
    IMPORT ATA_RESOURCE ataResources[];
/*	IMPORT UINT32 ioaddr1;
	IMPORT UINT32 ioaddr2;*/
...
#if 1
	ipAttach(0, "gei");
	ifMaskSet("gei0", 0xffffffe0);
	ifAddrSet("gei0", "10.0.0.105");
#endif
...
#ifdef INCLUDE_UART
	if(uartDrv() == ERROR){
		printf("uartDrv install error!\n");
	}
	uartDevCreate("/uart/1", 0);
	uartDevCreate("/uart/2", 1);
	uartDevCreate("/uart/3", 2);
#endif


#ifdef INCLUDE_ATA
	if(ataStatus)
		usrAtaConfig(0,0,"/ata0/"); /* added by yinwx, 2009-04-29 */
#endif
#if 0
	ipAttach(0, "gei");
	ifMaskSet("gei0", 0xffffffe0);
...
#if 1
	ipAttach(1, "gei");
	ifMaskSet("gei1", 0xffffffe0);
	ifAddrSet("gei1", "10.0.0.34");

	ipAttach(2, "gei");
	ifMaskSet("gei2", 0xffffffE0);
	ifAddrSet("gei2", "10.0.0.66");

	ipAttach(3, "gei");
	ifMaskSet("gei3", 0xffffffe0);
	ifAddrSet("gei3", "10.0.0.130");
#endif
