...
/* For SiI0680, added by yinwx, 2009-04-27 */
IMPORT UINT32 Sii0680_baseaddr;
IMPORT UINT32 Sii0680_ioaddr1;
IMPORT UINT32 Sii0680_ioaddr2;
IMPORT STATUS pciIntConnect	(VOIDFUNCPTR *vector, VOIDFUNCPTR routine,
			 int parameter);
...
STATUS ataDriveInit
    ...
    if (ataDevIdentify (ctrl, drive) != OK){
		/* printf("minik=IDENTIFY is ERROR!!!!!!!!!!!\n"); */
        goto driveInitExit;
		}
    ...
    if (pDrive->type == ATA_TYPE_ATA)
		...
		((pCtrl->ctrlType != ATA_PCMCIA) && (drive == 0)))
		{
            ...
            {printf("ataPread pParam is ERROR!!!!!!!!!!!\n");
...
STATUS ataDrv
	...
	if (wdgTimeout == 0)
        ...
        semBInit (&pCtrl->syncSem, SEM_Q_FIFO, /*SEM_FULL*/ SEM_EMPTY);
...
#endif
	if(ctrl==0){ /* added by yinwx, for 0680, 2009-04-27 */
		/*
	printf("###Sii0680_ioaddr1 is 0x%x####\n", Sii0680_ioaddr1);
	printf("###Sii0680_ioaddr2 is 0x%x####\n", Sii0680_ioaddr2);
	*/
	pCtrl->data	= ATA_DATA	(Sii0680_ioaddr1);
	pCtrl->error	= ATA_ERROR	(Sii0680_ioaddr1);
	pCtrl->feature	= ATA_FEATURE	(Sii0680_ioaddr1);
	pCtrl->seccnt	= ATA_SECCNT	(Sii0680_ioaddr1);
	pCtrl->sector	= ATA_SECTOR	(Sii0680_ioaddr1);
	pCtrl->cylLo	= ATA_CYL_LO	(Sii0680_ioaddr1);
	pCtrl->cylHi	= ATA_CYL_HI	(Sii0680_ioaddr1);
	pCtrl->sdh	= ATA_SDH	(Sii0680_ioaddr1);
	pCtrl->command	= ATA_COMMAND	(Sii0680_ioaddr1);
	pCtrl->status	= ATA_STATUS	(Sii0680_ioaddr1);
	pCtrl->aStatus	= ATA_A_STATUS	(Sii0680_ioaddr2); /* ioaddr2 offset +2 */
	pCtrl->dControl	= ATA_D_CONTROL (Sii0680_ioaddr2);
	pCtrl->dAddress	= ATA_D_ADDRESS (Sii0680_ioaddr2);
	}
	pciIntConnect(INUM_TO_IVEC(IV_PATA_IDE0), (VOIDFUNCPTR)ataIntr, 0);
	*(volatile unsigned long *)(0xbfe00130)=0x10;/* enable CF card ISR */
	#if 1
	tmp = *(volatile unsigned int *)(Sii0680_baseaddr + 0x04);
	tmp |= (0x01 << 1);
	*(volatile unsigned int *)(Sii0680_baseaddr + 0x04) = tmp;

	tmp = *(volatile unsigned int *)(Sii0680_baseaddr + 0x90);
	tmp |= (0x01 << 11);
	*(volatile unsigned int *)(Sii0680_baseaddr + 0x90) = tmp;	
	#endif
	#if 0
	if(level<16){
	...
	else{/* added by yinwx, for SiI0680, 2009-04-27 */
		pciIntConnect((VOIDFUNCPTR *)INUM_TO_IVEC (vector), 
					(VOIDFUNCPTR)ataIntr, ctrl);
		*(volatile unsigned long *)(0xbfe00130)=0x40;
	}
	#endif
	...
	pCtrl->installed = TRUE;
	    ...
	    pParam = &pDrive->param;
            ...
            if(ataDriveInit (ctrl, drive)==OK)
            	{
				/* printf("CTRL %d ataDriveInit is OK1!!!\n\n", ctrl); */
            	}
			else{
				/* printf("CTRL %d ataDriveInit is ERROR!!!\n\n", ctrl); */
				return ERROR;
			}
            }
...
LOCAL void ataIntr
    ...
    {
       ATA_CTRL *pCtrl	= &ataCtrl[ctrl];
	int isr = *(volatile unsigned int *)(Sii0680_baseaddr + 0x08);
	if((isr & 0x100) == 0)
		return;
/*logMsg("ATA is INTERRUPT!!!!!!!!\n",0,0,0,0,0,0);*/
      pCtrl->intCount++;
...
LOCAL STATUS ataPread
    ...
    while (retry)
	{
        ...
        if ((pCtrl->intStatus & ATA_STAT_ERR) || (semStatus == ERROR))
	    {
			...
			printf("ataPread%d/%d - err: status=0x%x intStatus=0x%x error=0x%x semStatus=%d\n", ctrl, drive, 
                          ATA_IO_BYTE_READ (pCtrl->aStatus), pCtrl->intStatus, 
                          ATA_IO_BYTE_READ (pCtrl->error), semStatus);
...
LOCAL STATUS ataDevIdentify
	...
	/*
	printf("pCtrl->seccnt = %x\n", pCtrl->seccnt);
	printf("ATAPI_STATUS = %x\n", ATAPI_STATUS);
 printf("In IDENTIFY: status is 0x%x\n", ATA_IO_BYTE_READ (ATAPI_STATUS));
 	*/
