...
typedef struct feiResource             /* FEI_RESOURCE */
    ...
    INT32            str[6+8];/*wangfq:+8 for cacheline forward align consideration*/           /* storage for the self-test result */
...
LOCAL UINT32  sys557LocalToBus(int unit, UINT32 localAdr);/*wangfq*/
LOCAL UINT32  sys557BusToLocal(int unit, UINT32 sysAdr);/*wangfq*/
...
IMPORT void cacheLsn2eFlushInvalidateAll(void); /*wangfq*/
...
STATUS sys557PciInit
    ...
    feiPciResources[feiUnits].bar[0]     = memIo32 | 0xa0000000; /*wangfq*/
    feiPciResources[feiUnits].bar[1]     = ioBase | 0xbfd00000; /*wangfq*/
    feiPciResources[feiUnits].bar[2]     = flash32 |0xa0000000; /*wangfq*/
    feiPciResources[feiUnits].irq        = irq;
    feiPciResources[feiUnits].irqvec     = /*INT_NUM_GET (irq)*/INUM_TO_IVEC(irq);/*wangfq*/
...
STATUS sys557Init
    ...
    if (pReso->initDone == FALSE)
		...
		/*wangfq temp*/
printf("eeprom 0,1,2:%.4x  %.4x  %.4x\n", pReso->eeprom[0], pReso->eeprom[1], pReso->eeprom[2]);
		/*pReso->eeprom[0]=0x0A00;
		
		pReso->eeprom[1]=0x30E4;
	
		pReso->eeprom[2]=0xA89E;*/
		...
		/*printf("pReso->str = %x   pReso->pResults = %x\n", pReso->str, pReso->pResults);*/
		
		pReso->pResults = (volatile INT32 *)((((int) pReso->str) + 0x1f) & ~0x1f);
	...
	#if 0	/*wangfq*/
		printf("pReso->pResults = %x\n",pReso->pResults);
		printf("before cacheLib.invalidateRtn\n"); /*wangfq*/
		cacheLsn2eFlushInvalidateAll();
		(* cacheLib.invalidateRtn)(DATA_CACHE,pReso->pResults,32);/*wangfq*/
		printf("pReso->pResults = %x\n",pReso->pResults);
	#endif 
        do  {
			...
			#if 1
			/*printf("pReso->pResults = %x\n",pReso->pResults);*/
			(cacheLib.invalidateRtn)(DATA_CACHE,pReso->pResults,1);/*wangfq*/
			/*printf("pReso->timeout = %d\n",pReso->timeout);*/
			/*cacheLsn2eFlushInvalidateAll();*/
			#endif
			/*pReso->pResults[1] = *(INT32 *)K0_TO_K1(pReso->pResults + 1);*/
        ...
        feiEndIntConnect    = (FUNCPTR) /*pciI*/pciIntConnect;
    ...
    pBoard->sysLocalToBus = /*NULL*/sys557LocalToBus; 
    pBoard->sysBusToLocal = /*NULL*/sys557BusToLocal; 
...
LOCAL STATUS sys557IntEnable
    ...
    { /*wangfq*/
		...
		printf("sys557IntEnable !!!!!!!!!!!!!!!!!!!!\n");
    	BONITO_INTENSET=0x10;/* original 0x00010000 */
    	return OK;
    /*
    return ((unit >= feiUnits) ? ERROR :
           (sysIntEnablePIC (feiPciResources[unit].irq))); 
    ## This is controlled by NB not SB, so PIC is not used! ##
    */
    }
...
/*wangfq*/
LOCAL UINT32 sys557LocalToBus(int unit,UINT32 localAdr)
{
	return (localAdr|0x80000000);
}

LOCAL UINT32 sys557BusToLocal(int unit,UINT32 sysAdr)
{
	return ((sysAdr&0x1fffffff)|0x80000000);
}
