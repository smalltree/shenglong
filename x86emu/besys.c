    ...
    u32 addr)
{
	...
	prom_printf("mem_read: address %#x out of range!\n", addr);
        HALT_SYS();
    }
DB( if (DEBUG_MEM())
        prom_printf("%#08x 1 -> %#x\n", addr, val);)
...
#ifdef DEBUG_EMU_VGA
	prom_printf("mem_write: address %#x out of range!\n", addr);
#endif
        HALT_SYS();
    }
}
...
#ifdef PRINT_PORT
	prom_printf(" inb(%#x) = %2.2x\n", port, val);
#endif
	return val;
    } 
}
{/* Start-- added for port 0x61! 2008-10-29 */
static unsigned short Int10Current_inb61time=0;
    if (port == 0x61) {
    Int10Current_inb61time++;
    val = (u8)(Int10Current_inb61time >>
              ((Int10Current_inb61time & 1) << 3));
#ifdef PRINT_PORT
    printf(" inb(%#x) = %2.2x\n", port, val);
#endif
    return val;
    }
}/* End-- added for port 0x61!! 2008-10-29 */
#endif
...
pciCfg1in(u16 addr, u32 *val,int type)
{
	...
	*val = PciCfg1Addr;
	...
	if(type==0){
		...
		pcibios_read_config_dword(BUS(PciCfg1Addr),DEVFN(PciCfg1Addr),OFFSET(PciCfg1Addr),(u32*)val);
  	   /*prom_printf("wrong type for pci config op\n");*/
	}
	return 1;
    }
    return 0;
