    ...
    int intno)
{
    if (BE_rdw(intno * 4 + 2) == BIOS_SEG)
        prom_printf("biosEmu: undefined interrupt %xh called!\n",intno);
    else
    ...
    int intno)
{
	...
	/* Not Implemented                                    */
	{                                         /* Localise */
	    prom_printf("Not implemented: Get Light Pen Position\n");
	...
	/* Not Implemented                                    */
	{                                         /* Localise */
	    prom_printf("Not implemented:Initialise or Scroll window up\n");
	}
	break;
	...
	/* Not Implemented                                    */
	{                                         /* Localise */
	    prom_printf("Not implemented:Write Character and Attibute at Cursor\n");
	}
	break;
	...
	/* Not Implemented                                    */
	{                                         /* Localise */
	    prom_printf("Not implemented:Write Character at Cursor\n");
	}
	break;
	...
	/* Not Implemented                                    */
	{                                         /* Localise */
	    prom_printf("Not implemented: Write Graphics Pixel\n");
	}
	break;
	...
	/* Not Implemented                                    */
	{                                         /* Localise */
	    prom_printf("Not implemented: Read Graphics Pixel\n");
	    M.x86.R_AL = 0;
	}
	break;
	...
	/*           installed yet.                           */
	{                                         /* Localise */
	    prom_printf("Not implemented: Write Character in Telebyte Mode\n");
	}
	break;
	...
	/*           installed yet.                           */
	{                                         /* Localise */
	    prom_printf("Not implemented: Write String in Telebyte Mode\n");
	}
	break;

    default:
        prom_printf("biosEmu/bios.int42: unknown function AH=%#02x, AL=%#02x, BL=%#02x\n",M.x86.R_AH, M.x86.R_AL, M.x86.R_BL);
	/* Various extensions                                 */
	/* Enter:  Various                                    */
	/* Leave:  Various                                    */
	/* Ignored                                            */
...
static void  int1A(int unused)
{
    ...
    if (!_BE_env.vgaInfo.pciInfo)
        return;
    pciSlot = (u16)((_BE_env.vgaInfo.pciInfo->bus<<8)|(_BE_env.vgaInfo.pciInfo->devnum<<3)|(_BE_env.vgaInfo.pciInfo->func&0x7));
#ifdef DEBUG_EMU_VGA
prom_printf("int 0x1a: ax=0x%x bx=0x%x cx=0x%x dx=0x%x di=0x%x es=0x%x\n",
    ...
    switch (M.x86.R_AX) {
        ...
        case 0xB102:                    /* Find PCI device */
            ...
            if (M.x86.R_DX == PCI_VENDOR(_BE_env.vgaInfo.pciInfo->pa_id) &&
                    M.x86.R_CX == _BE_env.vgaInfo.pciInfo->devnum &&
                    M.x86.R_SI == 0) {
        ...
        case 0xB103:                    /* Find PCI class code */
            M.x86.R_AH = DEVICE_NOT_FOUND;
            if (M.x86.R_ECX == _BE_env.vgaInfo.pciInfo->pa_class) {
               M.x86.R_AH = SUCCESSFUL;
               M.x86.R_BX = pciSlot;
            }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
        ...
        case 0xB108:                    /* Read configuration byte */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pciConfigInByte(_BE_env.vgaInfo.pciInfo->bus, _BE_env.vgaInfo.pciInfo->devnum, _BE_env.vgaInfo.pciInfo->func, M.x86.R_DI,&M.x86.R_CL);
		prom_printf("read_config_byte(0x%x)=0x%x\n",M.x86.R_DI, M.x86.R_CL);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        case 0xB109:                    /* Read configuration word */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pciConfigInWord(_BE_env.vgaInfo.pciInfo->bus,_BE_env.vgaInfo.pciInfo->devnum, _BE_env.vgaInfo.pciInfo->func, M.x86.R_DI,&M.x86.R_CX);
		prom_printf("read_config_word(0x%x)=0x%x\n",M.x86.R_DI, M.x86.R_CX);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        case 0xB10A:                    /* Read configuration dword */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pciConfigInLong(_BE_env.vgaInfo.pciInfo->bus,_BE_env.vgaInfo.pciInfo->devnum, _BE_env.vgaInfo.pciInfo->func, M.x86.R_DI,&M.x86.R_ECX);
		prom_printf("read_config_dword(0x%x)=0x%x\n",M.x86.R_DI, M.x86.R_ECX);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        case 0xB10B:                    /* Write configuration byte */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pciConfigOutByte(_BE_env.vgaInfo.pciInfo->bus,_BE_env.vgaInfo.pciInfo->devnum,_BE_env.vgaInfo.pciInfo->func,M.x86.R_DI,M.x86.R_CL);
		prom_printf("write_config_byte(0x%x)=0x%x\n",M.x86.R_DI, M.x86.R_CL);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        case 0xB10C:                    /* Write configuration word */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pciConfigOutWord(_BE_env.vgaInfo.pciInfo->bus, _BE_env.vgaInfo.pciInfo->devnum, _BE_env.vgaInfo.pciInfo->func, M.x86.R_DI,M.x86.R_CX);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        case 0xB10D:                    /* Write configuration dword */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pciConfigOutLong(_BE_env.vgaInfo.pciInfo->bus, _BE_env.vgaInfo.pciInfo->devnum, _BE_env.vgaInfo.pciInfo->func, M.x86.R_DI,M.x86.R_ECX);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        default:
            prom_printf("biosEmu/bios.int1a: unknown function AX=%#04x\n", M.x86.R_AX);
        }
}
...
{
	prom_printf("int E6: not implemented\n");
}
#if 1
static void  int15(int intno)
{
	...
	case 0x7f07:
		prom_printf("int 15:AX=%x\n",M.x86.R_AX);
		M.x86.R_AX=0x007f;
		pciConfigInWord(_BE_env.vgaInfo.pciInfo->bus,_BE_env.vgaInfo.pciInfo->devnum, _BE_env.vgaInfo.pciInfo->func,0x0,&M.x86.R_BX);
		pciConfigInWord(_BE_env.vgaInfo.pciInfo->bus,_BE_env.vgaInfo.pciInfo->devnum,_BE_env.vgaInfo.pciInfo->func,0x2,&M.x86.R_CX);
		break;
	case 0x7f08:
		prom_printf("int 15:AX=%x\n",M.x86.R_AX);
		M.x86.R_AX=0x007f;
	...
	case 0x7f14:
		...
		break;
         default:
		prom_printf("int 15:AX=%x is not emulation\n",M.x86.R_AX);
    }		
}
#endif
/****************************************************************************
...
fillin_key_datas(void)
{
    ...
    *((unsigned char *)(sys_base + 0x0FFFE)) = 0xfc;
    ...
    /*prom_printf("M.membase=%lx,BE_env.sysmem_base=%lx,sys_base=%p,mem_base=%p\n",M.mem_base,_BE_env.sysmem_base,sys_base,mem_base);*/
