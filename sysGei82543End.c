...
#define GEI_RXDES_NUM              (GEI_DEFAULT_RXDES_NUM << 6) /*<<3 by wangfq*/
#endif

/* Default TX descriptor  */

#ifndef GEI_TXDES_NUM
#define GEI_TXDES_NUM              (GEI_DEFAULT_TXDES_NUM << 5) 
#endif
...
#ifndef GEI_USR_FLAG
#define GEI_USR_FLAG        (GEI_END_SET_RX_PRIORITY)/*(GEI_END_SET_TIMER | \
...
#define GEI_OFFSET_VALUE            /*GEI_X86_OFFSET_VALUE*/GEI_ARM_OFFSET_VALUE /*wangfq note!!!!*/ 
...
#define PRO1000_541_PCI_DEVICE_ID_FT	(0x1078)
...
IMPORT	void printnum(unsigned long long n);
...
LOCAL UINT32	sys543LocalToBus(int, UINT32);
LOCAL UINT32	sys543BusToLocal(int, UINT32);
...
STATUS sys543PciInit
    ...
    pReso->memBaseLow  = /*memBaseLo;*/ memBaseLo | 0xa0000000; /*wangfq*/
    pReso->memBaseHigh = /*memBaseHi;*/ memBaseHi | 0xa0000000; /*wangfq*/
    pReso->flashBase   = /*flashBase;*/ flashBase | 0xa0000000; /*wangfq*/

    geiPciResources[geiUnits].irq        = irq;
    geiPciResources[geiUnits].irqvec     = (int)(INUM_TO_IVEC(irq));/*wangfq*/
...
END_OBJ * sysGei8254xEndLoad
    ...
    if (strlen (pParamStr) == 0)
		...
		/*
printf("Gei initstring is %s\nunit,shMemBase,shmemSize,rxDesNum,txDesNum,usrFlags,offset,mtu\n",paramStr);
		*/
...
STATUS sys82543BoardInit
    ...
    GEI_RESOURCE *       pReso;
    BOOL                 lanB = FALSE;
	/* printf("sys82543BoardInit start...\n"); *//*wangfq*/
    /* sanity check */
...
/*wangfq*/
#if 0
pReso->enetAddr[0]= 0x00;
pReso->enetAddr[1]= 0x0A;
pReso->enetAddr[2]= 0xE4;
pReso->enetAddr[3]= 0x30;
pReso->enetAddr[4]= 0x9E;
pReso->enetAddr[5]= 0xAA+unit;/*unit0xA9 modified by yinwx*/
#endif
#if 1
 	if (sys543EtherAdrGet (unit) == OK){
    ...
    pBoard->vector      = /*INT_NUM_GET((pRsrc->irq));*/pRsrc->irq;/*wangfq*/
    pBoard->regBaseHigh = pReso->memBaseHigh;
    pBoard->regBaseLow  = /*PCI_MEMIO2LOCAL(pReso->memBaseLow)*/pReso->memBaseLow;
    pBoard->flashBase   = /*PCI_MEMIO2LOCAL(pReso->flashBase)*/pReso->flashBase;
    ...
    pBoard->phyDelayRtn = (FUNCPTR) taskDelay/*NULL*/;
    pBoard->phyMaxDelay = MII_PHY_DEF_DELAY; /*wangfq*/
    ...
    if (pRsrc->boardType == PRO1000_546_BOARD)
        {
        pBoard->sysGeiDynaTimerSetup = NULL;/*sysGei82546DynaTimerSetup,wangfq*/
        pBoard->sysGeiInitTimerSetup = NULL;/*sysGei82546InitTimerSetup,wangfq*/ 
        }
    else
        {
    ...
    pBoard->sysLocalToBus = sys543LocalToBus;/*NULL;*/	/* modified by yinwx, original NULL for 1:1 mapping */ 
    pBoard->sysBusToLocal = sys543BusToLocal;/*NULL;*/	/* modified by yinwx, origianl NULL for 1:1 mapping */ 
...
pReso->eeprom_icw1 = 0x460B; /*wangfq*/ /*0x4408*/
pReso->eeprom_icw2 = 0x34E8; /*wangfq*/	/*0x3200*/
...
#if 0
sys543Show(unit); /*wangfq*/
#endif
    /* enable adaptor interrupt */
    /*sysIntEnablePIC (pRsrc->irq);*/ /*wangfq*/
	sys543IntEnable(unit);
/* printf(" end...\n"); *//*wangfq*/
    return (OK);
    }
...
LOCAL BOOL sysGei82546DynaTimerSetup 
    ...
    /* value > 0xffff would not be used for corresponding timer registers */
     ...
     pBoard->devTimerUpdate.rdtrVal = 0x8000; /* unit of 1.024us */
     pBoard->devTimerUpdate.radvVal = 0x8000; /* unit of 1.024us */
     pBoard->devTimerUpdate.itrVal  = 0x80; /* unit of 256 ns */
...
LOCAL BOOL sysGei82546InitTimerSetup 
    ...
    /* value > 0xffff would not change corresponding timer registers */
     ...
     pBoard->devTimerUpdate.rdtrVal = 0x8000; /* unit of 1.024ns */
     pBoard->devTimerUpdate.radvVal = 0x8000; /* unit of 1.024us */
     pBoard->devTimerUpdate.itrVal  = 0x80; /* unit of 256 ns */
     pBoard->devTimerUpdate.watchDogIntVal = 2;   /* 2 second default */
...
/*****************************************************************************************/
    ...
    for (count = 0; count < 2; count++)
         sysDelay ();
    sys543eepromWriteBits (unit, 0x13, 0x5);
	sys543eepromWriteBits(unit, 0x60, 8);
...
/*****************************************************************************************/
    ...
    GEI_SYS_WRITE_REG(unit, INTEL_82543GC_EECD, EECD_CS_BIT|0x20);
   ...
   /* val = sys543eepromReadBits (unit, EEPROM_DATA_BITS);*/
	sys543eepromWriteBits(unit, data, EEPROM_DATA_BITS);
...
{	
	sys543eepromWriteWord(unit, 0x00, addr0);
    	myDelay();
	sys543eepromWriteWord(unit, 0x01, addr1);
    	myDelay();
	sys543eepromWriteWord(unit, 0x02, addr2);
	myDelay();
...
STATUS eepromShow
    ...
    for (ix = 0; ix < EEPROM_WORD_SIZE; ix++){
        sum = sys543eepromReadWord (unit, ix);
	printf("sum = 0x%x\n", sum);
    	}
...
LOCAL void sys543PhySpecRegsInit
	...
	taskLock();
	printstr("sys543PhySpecRegsInit\r\n");
    MII_READ (phyAddr, MII_PHY_ID1_REG, &phyId1, retVal);
       printnum(phyId1);
    MII_READ (phyAddr, MII_PHY_ID2_REG, &phyId2, retVal);
       printnum(phyId2);
    ...
    phyMode = (phyId2 & MII_ID2_MODE_MASK) >> 4; /* 0x02 */
...
LOCAL STATUS sys543IntEnable
	...
	if(unit == 0)
		{
    	BONITO_INTENSET=0x10;
			return (OK);
		}
		
	if(unit == 1)
		{
    	BONITO_INTENSET=0x20;
			return (OK);
		}

	if(unit == 2)
		{
    	BONITO_INTENSET=0x40;
			return (OK);
		}

	if(unit == 3)
		{
    	BONITO_INTENSET=0x80;
			return (OK);
		}
	return ERROR;
    }
...
LOCAL STATUS sys543IntDisable
	...
	if(unit == 0)
		{
    	BONITO_INTENCLR = 0x10;
			return (OK);
		}
		
	if(unit == 1)
		{
    	BONITO_INTENCLR = 0x20;
			return (OK);
		}

	if(unit == 2)
		{
    	BONITO_INTENCLR = 0x40;
			return (OK);
		}

	if(unit == 3)
		{
    	BONITO_INTENCLR = 0x80;
			return (OK);
		}
...
/*****************************************************************************
*
* sys543LocalToBus - local-to-bus address translation, added by yinwx, 2009-03-04
*
* This routine map local address to bus address,
* will be attached to pBoard->sysLocalToBus routine
*
* RETURNS: Bus address
*/
UINT32 sys543LocalToBus
	(
	int		unit,
	UINT32	localadrs
	)
	{
	return (UINT32)(localadrs | 0x80000000);
	}

/*****************************************************************************
*
* sys543BusToLocal - bus-to-local address translation, added by yinwx, 2009-03-04
*
* This routine map bus address to local address,
* will be attached to pBoard->sysBusToLocal routine
*
* RETURNS: Bus address
*/
UINT32 sys543BusToLocal
	(
	int		unit,
	UINT32	busadrs
	)
	{
	return (UINT32)(busadrs & (~0x80000000));
	}
